    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden sm:flex sm:items-center sm:ml-6">
                    <x-dropdown align="left" width="48">
                        ...
                        <x-slot name="content">
                            ...
                            @if(!request()->routeIs('chatrooms.show', session('chat_room_id')))
                                <x-dropdown-link href="{{ route('chatrooms.show', session('chat_room_id')) }}">
                                    Aktuellen Raum öffnen
                                </x-dropdown-link>
                            @endif
                            <form method="POST" action="{{ route('chatrooms.leave') }}">
                                @csrf
                                <x-dropdown-link href="{{ route('chatrooms.leave') }}"
                ...
                @else
                <div class="hidden sm:flex sm:items-center sm:ml-6">
                    <a href="{{ route('chatrooms.index') }}" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:text-gray-500 focus:outline-none transition ease-in-out duration-150">
                        Raumliste
                    </a>
                </div>
                @endif
